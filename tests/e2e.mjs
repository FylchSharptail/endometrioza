...
L('\n===== DESKTOP (1440x900) =====');
{
  ...
  // Drag the pill to move it, then reopen menu to test relative positioning
  const snBox = await page.locator('#sn').boundingBox();
  await page.hover('#sn');
  await page.waitForTimeout(400);
  // drag grip
  const grip = await page.locator('#sn-grip').boundingBox();
  await page.mouse.move(grip.x + grip.width/2, grip.y + grip.height/2);
  await page.mouse.down();
  await page.mouse.move(300, 400, {steps: 10});
  await page.mouse.up();
  await page.waitForTimeout(400);
  await shot(page, 'desktop-4-pill-moved');
  await page.click('#sn-menu');
  await page.waitForTimeout(400);
  await shot(page, 'desktop-5-menu-after-move');
  await measure(page, 'desktop menu after pill moved');
