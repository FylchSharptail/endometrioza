...
showBars();
var tipEl=null;
function ensureTip(){if(!tipEl){tipEl=document.createElement('div');tipEl.className='tip-portal fixed bg-ink-2 text-surface [padding:8px_12px] rounded-[7px] text-[11.5px] max-w-[260px] leading-[1.4] opacity-[0] pointer-events-none [transition:opacity_180ms_cubic-bezier(.22,.9,.3,1),_transform_320ms_cubic-bezier(.34,1.3,.64,1)] z-[2000] font-medium shadow-[0_6px_20px_rgba(0,_0,_0,_0.55)] text-center border-[1px] border-[#2a2f3a] [transform:scale(0.82)] [transform-origin:center] before:content-[""] before:absolute before:left-[50%] before:ml-[-6px] before:border-[6px] before:border-[transparent] before:w-[0] before:h-[0] [&.bottom]:[transform-origin:center_top] [&.bottom]:before:bottom-[100%] [&.bottom]:before:border-b-ink-2 [&.left]:[transform-origin:right_center] [&.left]:before:left-[100%] [&.left]:before:top-[50%] [&.left]:before:ml-[0] [&.left]:before:mt-[-6px] [&.left]:before:border-[transparent] [&.left]:before:border-l-ink-2 [&.show]:opacity-[1] [&.show]:[transform:scale(1)] [&.top]:[transform-origin:center_bottom] [&.top]:before:top-[100%] [&.top]:before:border-t-ink-2';document.body.appendChild(tipEl)}}
function showTip(el){ensureTip();var text=el.getAttribute('data-tip')||el.getAttribute('aria-label')||'';if(!text)return;tipEl.textContent=text;var r=el.getBoundingClientRect();tipEl.classList.remove('top','bottom','left','show');var below=(innerHeight-r.bottom)>r.top;tipEl.classList.add(below?'bottom':'top');tipEl.style.visibility='hidden';tipEl.classList.add('show');var tr2=tipEl.getBoundingClientRect();var left=r.left+r.width/2-tr2.width/2;left=Math.max(8,Math.min(left,innerWidth-tr2.width-8));tipEl.style.left=left+'px';tipEl.style.top=(below?(r.bottom+10):(r.top-tr2.height-10))+'px';tipEl.style.visibility=''}
...
function closeMenu(){snList.classList.remove('open');snMenu.classList.remove('on');sn.classList.remove('open');snList.setAttribute('aria-hidden','true')}
document.addEventListener('keydown',function(e){if(e.key==='Escape'||e.keyCode===27){var anyOpen=snList.classList.contains('open');if(anyOpen){e.preventDefault();closeMenu();snMenu.focus()}}});
...
function posSnList(){var r=sn.getBoundingClientRect();var listH=snList.offsetHeight||Math.min(innerHeight*.7,420);var spaceBelow=innerHeight-r.bottom,spaceAbove=r.top;var above=spaceBelow<listH+16 && spaceAbove>spaceBelow;snList.classList.toggle('above',above)}snMenu.addEventListener('click',function(e){e.stopPropagation();var o=!snList.classList.contains('open');snList.classList.toggle('open',o);snMenu.classList.toggle('on',o);sn.classList.toggle('open',o);snList.setAttribute('aria-hidden',o?'false':'true');if(o){posSnList();var firstA=snList.querySelector('a');if(firstA&&e.detail===0)firstA.focus()}});window.addEventListener('resize',function(){if(snList.classList.contains('open'))posSnList()});window.addEventListener('scroll',function(){if(snList.classList.contains('open'))posSnList()},{passive:true});
...
var snD=null;
function snStart(e){e.preventDefault();e.stopPropagation();var p=gxy(e),r=sn.getBoundingClientRect();sn.style.left=r.left+'px';sn.style.top=r.top+'px';sn.style.right='auto';sn.classList.add('moved');snD={ox:p.x-r.left,oy:p.y-r.top,shift:0};sn.style.transition='none'}
