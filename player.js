...
function posSnList(){var r=sn.getBoundingClientRect();var lw=snList.offsetWidth||240;var lh=snList.offsetHeight||Math.min(innerHeight*.7,420);var left=r.left-lw-10;if(left<8)left=r.right+10;if(left+lw>innerWidth-8)left=Math.max(8,innerWidth-lw-8);var top=r.top+r.height/2-lh/2;top=Math.max(8,Math.min(top,innerHeight-lh-8));snList.style.left=left+'px';snList.style.top=top+'px';snList.style.right='auto';snList.style.transform='translateX(0)'}snMenu.addEventListener('click',function(e){e.stopPropagation();var o=!snList.classList.contains('open');snList.classList.toggle('open',o);snMenu.classList.toggle('on',o);sn.classList.toggle('open',o);if(o)posSnList()});window.addEventListener('resize',function(){if(snList.classList.contains('open'))posSnList()});window.addEventListener('scroll',function(){if(snList.classList.contains('open'))posSnList()},{passive:true});
