...
function posSnList(){var r=sn.getBoundingClientRect();var lw=snList.offsetWidth||240;var lh=snList.offsetHeight||Math.min(innerHeight*.7,420);var gap=10;var pillCx=r.left+r.width/2,pillCy=r.top+r.height/2;var preferLeft=pillCx>innerWidth/2;var left=preferLeft?r.left-lw-gap:r.right+gap;if(preferLeft&&left<8)left=r.right+gap;else if(!preferLeft&&left+lw>innerWidth-8)left=r.left-lw-gap;left=Math.max(8,Math.min(left,innerWidth-lw-8));var top=pillCy-lh/2;top=Math.max(8,Math.min(top,innerHeight-lh-8));var side=left<pillCx?'left':'right';snList.classList.toggle('from-right',side==='right');snList.classList.toggle('from-left',side==='left');snList.style.left=left+'px';snList.style.top=top+'px';snList.style.right='auto'}snMenu.addEventListener('click',function(e){e.stopPropagation();var o=!snList.classList.contains('open');snList.classList.toggle('open',o);snMenu.classList.toggle('on',o);sn.classList.toggle('open',o);if(o)posSnList()});window.addEventListener('resize',function(){if(snList.classList.contains('open'))posSnList()});window.addEventListener('scroll',function(){if(snList.classList.contains('open'))posSnList()},{passive:true});
